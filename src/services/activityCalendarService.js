...
const STORAGE_KEY = "happy_state_activity_calendar_v2";
...
export async function getActivityCalendarEntries(modeOverride = null) {
  ...
  const localEntries = sortEntries(
    (readJson(storageKey, []) || []).map(normalizeCalendarEntry).filter(Boolean),
  );
  ...
  try {
    ...
    const remoteEntries = Array.isArray(snap.data()?.activityCalendar?.entries)
      ? snap.data().activityCalendar.entries.map(normalizeCalendarEntry).filter(Boolean)
      : [];
    ...
    writeJson(storageKey, sortedRemote);
    return sortedRemote;
  } catch {
    return localEntries;
  }
...
export async function saveActivityCalendarEntries(entries, modeOverride = null) {
  const mode = normalizeCharacterMode(modeOverride || getActiveCharacterMode());
  const normalized = sortEntries((entries || []).map(normalizeCalendarEntry).filter(Boolean));
  writeJson(getStorageKey(mode), normalized);
  const userDocId = getUserDocId(auth.currentUser || {});
  if (userDocId) {
    try {
      const ref = activityCalendarRef(userDocId, mode);
...
export async function upsertActivityCalendarEntry({ date, score }, modeOverride = null) {
  ...
  else existing.push({ ...nextEntry, createdAt: nowISO });
...
export async function deleteActivityCalendarEntry(date, modeOverride = null) {
  const safeDate = toDateKey(date || new Date());
  const existing = await getActivityCalendarEntries(modeOverride);
  return saveActivityCalendarEntries(
    existing.filter((item) => item.date !== safeDate),
    modeOverride,
  );
}
