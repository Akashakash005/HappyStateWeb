...
function normalizePerson(person = {}) {
  return {
    ...
    pressureCount: Number(person.pressureCount || 0),
    positiveCount: Number(person.positiveCount || 0),
    neutralCount: Number(person.neutralCount || 0),
    ...
    pressureRatio: Number(person.pressureRatio || 0),
    positiveRatio: Number(person.positiveRatio || 0),
    interactionScore: Number(person.interactionScore || 0),
    interactionIntensity: Number(person.interactionIntensity || 0),
...
function applyManualEdits(circleState) {
  ...
  return {
    ...circleState,
    people,
    positiveEnergy: people.filter((person) => person.positiveRatio >= 0.5).map((person) => person.person),
    stressCorrelated: people.filter((person) => person.pressureRatio >= 0.5).map((person) => person.person),
