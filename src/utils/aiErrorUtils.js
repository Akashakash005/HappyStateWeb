...
function extractRetrySeconds(message = "", details = []) {
  const retryInfo = Array.isArray(details) ? details.find((item) => item?.["@type"]?.includes("RetryInfo")) : null;
...
export function getAiQuotaErrorDetails(error) {
  ...
  if (!looksLikeQuota) {
    return { isQuotaError: false, retrySeconds: null, message: "" };
  }
  const retrySeconds = extractRetrySeconds(message, payload?.details);
  const retryText = retrySeconds ? `Try again in about ${retrySeconds} seconds.` : "Try again after some time.";
  return {
    isQuotaError: true,
    retrySeconds,
