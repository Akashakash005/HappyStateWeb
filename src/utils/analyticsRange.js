...
export function getDateRange(filter, referenceDate = new Date(), customRange = null) {
  ...
  if (filter === "day") return { start: startOfDay(ref), end: endOfDay(ref) };
  if (filter === "week") return { start: startOfWeekMonday(ref), end: endOfWeekSunday(ref) };
  if (filter === "month") return { start: startOfMonth(ref), end: endOfMonth(ref) };
  if (filter === "halfyear") {
    const half = getHalfYearRange(ref);
    return { start: half.start, end: half.end };
  }
  if (filter === "custom") {
    const rawStart = customRange?.startDate ? startOfDay(customRange.startDate) : startOfDay(ref);
    const rawEnd = customRange?.endDate ? endOfDay(customRange.endDate) : endOfDay(ref);
