...
function summarizeCompact(entries, range) {
  ...
  const overallAverage = sentiments.length > 0 ? round2(sentiments.reduce((acc, s) => acc + s, 0) / sentiments.length) : 0;
  ...
  sorted.forEach((e) => {
    ...
    if (val < 0) negativeBuckets[getTimeBucket(e.dateISO)] += 1;
  });
...
export function compressYearToQuarterly(yearSummary) {
  ...
  (yearSummary.monthlyAverages || []).forEach((m) => {
    ...
    const key = `${year}-Q${q}`;
    qMap[key] = qMap[key] || { sum: 0, count: 0 };
  ...
  return { ...yearSummary, monthlyAverages: undefined, quarterlyAverages };
}
...
export function getMoodDataByRange(allEntries, range) {
  ...
  if (range === "day") return summarizeCompact(entries.filter((e) => toDateKey(e.dateISO) === todayKey), "day");
  if (range === "week") return summarizeCompact(filterEntriesByDays(entries, 7), "week");
  if (range === "month") return summarizeCompact(filterEntriesByDays(entries, 30), "month");
